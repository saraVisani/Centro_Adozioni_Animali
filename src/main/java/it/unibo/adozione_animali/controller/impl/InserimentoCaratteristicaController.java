...
public class InserimentoCaratteristicaController {
    private final Model model;
    private final Inserimento view;

    public InserimentoCaratteristicaController(Model model, Inserimento view) {
        this.model = model;
        this.view = view;
        inizializza();
    }
    ...
    public void salvaInserimento() {
        ...
        if (nuovo) {
            esito = model.getCaratteristicaPersonaliDAO()
                    .insertMultiplo(provincia, citta, Integer.valueOf(numero), codAnimale, tipo, codiciSpecifici);
        } else {
            esito = model.getAppartenenzaDAO()
                    .insertAppartenenza(codiciSpecifici, provincia, citta, Integer.valueOf(numero), codAnimale, tipo);
        }
