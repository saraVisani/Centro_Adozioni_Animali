...
public class ShowRichieste extends JPanel {

    public ShowRichieste() {
        ...
        search.addActionListener(e -> {
            try {
                String[] columns = { "Codice Animale", "Codice Fiscale Richiedente",
                        "Data Apertura", "Data Chiusura"};
                DefaultTableModel model = new DefaultTableModel(columns, 0);
                table.setModel(model);
                List<RichiestaRecord> requests = new RichiestaDAO().getRequests(codProvF.getText(), codCittaF.getText(),
                        Integer.parseInt(numeroF.getText()));
                requests.forEach(request -> {
                    Object[] row = {
                            request.getCodAnimale(),
                            request.getCf(),
                            request.getDataRichiesta(),
                            request.getDataChiusura()
                    };
                    model.addRow(row);
                });
